...
        [Header("Audio")]
        public SurfaceProfile surfaceProfile;
...
            {
                Vector3 swimDir = GameView.instance.transform.TransformDirection(Vector3.forward);
...
            float camHeight = GameView.instance.transform.position.y;
...
                    length = isCrouching ? crouchLen : (isRunning ? runLen : walkLen);
                }
            }
...
            bool canPlay = (isGrounded || isClimbing) || (isSwiming && !inputs.run.hold);

            if(controller.velocity.magnitude >= 0.1f && !isSliding && canPlay)
            {
...
            GameView.instance.FPSViewSettings.mainCharacter = controller;
            GameView.instance.ChangeView(CameraType.FPSView);                       
        }

        [Command] public void PlayAudioCMD(int surface)
        {
            PlayAudioRPC(surface);
        }

        [ClientRpc(includeOwner =false)] 
        public void PlayAudioRPC(int surface)
        {
            List<AudioClip> footsteps = surfaceProfile.GetFootStepSound(surface);

            if (footsteps == null) return;

            int n = Random.Range(1, footsteps.Count);
            footstepSource.pitch = Random.Range(0.9f, 1.1f); /// now same sound won't be heard the same
            footstepSource.PlayOneShot(footsteps[n]);     
        }
        #endregion
    }
